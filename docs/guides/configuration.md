...
hibernate_max_live = 0    # 0 disables; e.g. 10 = keep at most 10 idle sessions live (LRU)
...
| `hibernate_max_live` | `0` | Cap on plain sessions kept live at once. When more than this many have a running tmux/agent process, the least-recently-used idle ones are hibernated: tmux and any sandbox container are torn down, the row stays in every list marked dormant, and selecting it restarts the agent with its conversation resumed (the same `--resume` path a dead session's ensure uses). Working sessions and sessions with an attached tmux client are never hibernated, so the live count may exceed the cap while agents are busy. `0` disables it. Enforced by the `aoe serve` daemon about once a minute. |
